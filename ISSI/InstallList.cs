...
namespace ISSI
{
    public partial class InstallList : Form
    {
        private int userID;
       // DataTable modified;
        bool filter = false;
        ...
        public InstallList()
        {
            ...
            if (Session.loaded)
            {
                ...
                if (userID != 3)
                {
                    this.vInstalldbSiteListTableAdapter.FillByOpsID(this.sbi_installdbDataSet.vInstalldbSiteList, userID);
                    this.vInstallLineSummaryTableAdapter.FillByOpsID(this.sbi_installdbDataSet.vInstallLineSummary, userID);
                }
                else
                {
                    this.vInstalldbSiteListTableAdapter.Fill(this.sbi_installdbDataSet.vInstalldbSiteList);
                    this.vInstallLineSummaryTableAdapter.Fill(this.sbi_installdbDataSet.vInstallLineSummary);
                }
               // this.sbi_installdbDataSet.vInstallLineSummary.OrderBy(row => row.Field<DateTime?>("InstallDate"));
        ...
        {
            // TODO: This line of code loads data into the 'sbi_installdbDataSet.vInstallLineSummary' table. You can move, or remove it, as needed.
            //this.vInstallLineSummaryTableAdapter.Fill(this.sbi_installdbDataSet.vInstallLineSummary);
            // TODO: This line of code loads data into the 'sbi_installdbDataSet.vInstalldbSiteList' table. You can move, or remove it, as needed.
           // this.vInstalldbSiteListTableAdapter.Fill(this.sbi_installdbDataSet.vInstalldbSiteList);
        ...
        {
            string f = this.vInstallLineSummaryBindingSource.Filter;
            ...
            if (!(e.RowIndex == -1 || e.ColumnIndex == -1))
            {
                ...
                if (lineDataGridView.Columns[e.ColumnIndex].Name == "Site")
                {
                    ...
                    if (dt != null && wcPicker.Value.DayOfWeek == DayOfWeek.Monday)
                    {
                        if (userID != 3)
                        {
                            this.vInstallLineSummaryTableAdapter.FillByOpsPlannerInstallDate(this.sbi_installdbDataSet.vInstallLineSummary, Convert.ToString(wcPicker.Value), userID);

                        }
                        else
                        {

                            this.vInstallLineSummaryTableAdapter.FillBy(this.sbi_installdbDataSet.vInstallLineSummary, Convert.ToString(wcPicker.Value));
                        }
                ...
                this.vInstallLineSummaryBindingSource.Filter = f;
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            using (FilterByDialog fbd = new FilterByDialog("RAG", "RAG", lineDataGridView))
            {
                ...
                if (result == DialogResult.OK)
                {
                    ...
                    if (String.IsNullOrWhiteSpace(this.vInstallLineSummaryBindingSource.Filter))
                    {
                        this.vInstallLineSummaryBindingSource.Filter = "(" + filter + ")";
                    }
                    else
                    {
                        this.vInstallLineSummaryBindingSource.Filter += " AND (" + filter + ")";
                    }
        ...
        {
            vInstallLineSummaryBindingSource.RemoveFilter();
        ...
        private void InstallList_Shown(object sender, EventArgs e)
        {
        ...
        {
            DialogResult dialogResult = MessageBox.Show("Mark all selected lines as installed?", "Complete Job", MessageBoxButtons.YesNo);
            if (dialogResult == DialogResult.Yes)
            {
                if (lineDataGridView.SelectedRows.Count > 0)
                {
                    foreach (DataGridViewRow r in lineDataGridView.SelectedRows)
                    {
                        int id = Convert.ToInt32(r.Cells["InstallID"].Value);
                        this.installScheduleTableAdapter1.FillByInstallID(this.sbi_installdbDataSet.InstallSchedule, id);
                        this.sbi_installdbDataSet.InstallSchedule.FindByInstallID(id).StatusID = 3;
        ...
        public void RefreshAfterAdd(string filter)
        {
            ...
            if (!String.IsNullOrWhiteSpace(filter))
            {
                if (!String.IsNullOrWhiteSpace(this.vInstallLineSummaryBindingSource.Filter))
                {
                    this.vInstallLineSummaryBindingSource.Filter = "(" + this.vInstallLineSummaryBindingSource.Filter + ") OR (" + filter + ")";
                }
            }
            int pos = this.lineDataGridView.FirstDisplayedScrollingRowIndex;
        ...
        private void clientDataGridView_CellMouseClick(object sender, DataGridViewCellMouseEventArgs e)
        {
            if (!(e.RowIndex == -1 || e.ColumnIndex == -1))
            {
                if (filter)
                {
                    ...
                    string client = Convert.ToString(clientDataGridView.Rows[e.RowIndex].Cells["ClientCol"].Value);
                    if (clientDataGridView.Columns[e.ColumnIndex].Name == "SiteCol")
                    {
                    ...
                    if (clientDataGridView.Columns[e.ColumnIndex].Name == "ClientCol")
                    {
                    ...
                    if (!String.IsNullOrWhiteSpace(criteria))
                    {
                        ...
                        button2.Enabled = true;
                        button3.Enabled = true;
                        this.vInstallLineSummaryBindingSource.Filter = "(" + criteria + ")";
        ...
        private void countShows()
        {
            if (vInstallLineSummaryBindingSource.List.Count >0)
            {
                try
                {
                    DataView dv = (DataView)vInstallLineSummaryBindingSource.List;
                    DataTable ds = dv.ToTable();

                    DataTable dt = ds.AsEnumerable().Where(row => (row.Field<int>("JobType") == 1 || row.Field<int>("JobType") == 10)).CopyToDataTable();
                    labelShows.Text = "Showhomes: " + dt.Rows.Count.ToString();
                    labelDevs.Text = "Developments: " + dt.AsEnumerable().Select(l => l.Field<int>("SiteID")).Distinct().Count().ToString();
                }
                catch (Exception)
                {

                    labelShows.Text = "Showhomes: ";
                    labelDevs.Text = "Developments: ";
                }
            }
        }
